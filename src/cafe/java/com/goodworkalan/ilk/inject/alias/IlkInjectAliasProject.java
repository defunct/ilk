package com.goodworkalan.ilk.inject.alias;

import com.goodworkalan.cafe.ProjectModule;
import com.goodworkalan.cafe.builder.Builder;
import com.goodworkalan.cafe.outline.JavaProject;

/**
 * Builds the project definition for Ilk Inject Alias.
 *
 * @author Alan Gutierrez
 */
public class IlkInjectAliasProject implements ProjectModule {
    /**
     * Build the project definition Ilk Inject Alias.
     *
     * @param builder
     *          The project builder.
     */
    public void build(Builder builder) {
        builder
            .cookbook(JavaProject.class)
                .produces("com.github.bigeasy.ilk/ilk-inject-alias/0.1")
                .depends()
                    .production("com.github.bigeasy.ilk/ilk-inject/0.+1")
                    .development("org.testng/testng-jdk15/5.10")
                    .end()
                .end()
            .end();
    }
}
